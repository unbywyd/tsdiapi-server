...
export class SchemaRegistry {
     ...
     */
    extractDependencies(schema, dependencies = new Set(), visited = new Set()) {
        ...
        const typeboxRef = schema[Symbol.for('TypeBox.Ref')];
...
// Implementation
export function useSchema(schema, nameOrOptions) {
    ...
    if (!nameOrOptions) {
        ...
        throw new Error('useSchema: Schema must have $id property when used without name/options. ' +
    ...
    const { id, name, validate = true, checkDependencies = true, strict = true } = options;
    ...
    if (id) {
        ...
        throw new Error('useSchema: Schema ID is required. ' +
    ...
    try {
        const registry = getSchemaRegistry();
        const existingSchema = registry.fastify.getSchema(schemaId);
        if (existingSchema) {
            ...
            if (strict) {
                ...
                if (!areSchemasEquivalent(existingSchema, schema)) {
                    throw new Error(`Schema "${schemaId}" is already registered with a different definition. ` +
    ...
    try {
        ...
        if (checkDependencies) {
            ...
            for (const depId of deps) {
                ...
                if (!depSchema) {
                    console.warn(`Schema "${schemaId}" references "${depId}" which is not yet registered. ` +
 ...
 */
export function getSchemaRegistry(fastify) {
 ...
 */
export function initializeSchemaRegistry(fastify) {
    globalRegistry = new SchemaRegistry(fastify);
    return globalRegistry;
}
/**
 ...
 */
export async function autoRegisterSchemas(fastify, apiDir) {
