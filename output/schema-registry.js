...
export class SchemaRegistry {
    ...
    addSchema(schema) {
        ...
        if (!schema || !schema.$id || typeof schema.$id !== 'string') {
            throw new Error('Schema must have $id property. ' +
    ...
    refSchema(schemaId) {
        if (!schemaId || typeof schemaId !== 'string') {
            throw new Error(`refSchema() requires a string schema ID. ` +
    ...
    isResponseWrapper(schema) {
        ...
        const hasStatus = props.status &&
            (props.status.const !== undefined || props.status.enum !== undefined);
     ...
     */
    isPrismaGeneratedSchema(schema) {
        if (!schema || typeof schema !== 'object') {
            return false;
        }
        const schemaId = schema.$id;
        ...
        const props = schema.properties;
        if (props && typeof props === 'object') {
            const hasPrismaFields = (props.id !== undefined) &&
     ...
     */
    findDuplicateSchemas(newSchema) {
        const duplicates = [];
    ...
    register(schema) {
        ...
        if (this.options.logDuplicateSchemas) {
            ...
            if (duplicates.length > 0) {
                ...
                console.warn(`⚠️  Duplicate schema structure detected!\n` +
 ...
 */
export function extractSchemasFromModule(module) {
    const schemas = [];
    ...
    for (const key in module) {
        ...
        if (value && typeof value === 'object' && value.$id && typeof value.$id === 'string') {
            ...
            const hasTypeBoxStructure = value[Symbol.for('TypeBox.Kind')] !== undefined ||
                'type' in value ||
                'properties' in value ||
                'items' in value ||
                '$ref' in value;
 ...
 */
function validateSchemaStructure(schema, schemaId) {
    ...
    const hasTypeBoxStructure = 'type' in schema ||
        'properties' in schema ||
        'items' in schema ||
        '$ref' in schema ||
        (typeboxKind in schema && schema[typeboxKind] !== undefined);
    if (!hasTypeBoxStructure && !('$ref' in schema)) {
        console.warn(`Schema "${schemaId}" might not be a valid TypeBox schema. ` +
 ...
 */
function getSchemaPrefix(schemaId) {
 ...
 */
function areSchemasStructurallyEquivalent(schema1, schema2, visited = new Set()) {
    ...
    const normalize = (schema, depth = 0) => {
        ...
        if (schema.anyOf || schema.oneOf) {
            ...
            return {
                type: schema.type,
                [unionKey]: schema[unionKey].map((item) => normalize(item, depth + 1))
            };
        }
        ...
        for (const key of keys) {
            ...
            if (Array.isArray(value)) {
                normalized[key] = value.map(item => item && typeof item === 'object' ? normalize(item, depth + 1) : item);
                continue;
            }
 ...
 */
export function getSchemaRegistry(fastify, options) {
 ...
 */
export function initializeSchemaRegistry(fastify, options) {
    globalRegistry = new SchemaRegistry(fastify, options);
    // Register all pending schemas
    if (pendingSchemas.length > 0) {
        console.log(`📦 Registering ${pendingSchemas.length} early-loaded schema(s)...`);
 ...
 * ```
 */
export function refSchema(schemaId) {
    if (!schemaId || typeof schemaId !== 'string') {
        throw new Error(`refSchema() requires a string schema ID. ` +
            `Received: ${typeof schemaId}. ` +
            `Example: refSchema<typeof MySchema>('MySchema')`);
 ...
 */
export async function autoRegisterSchemas(fastify, apiDir, options) {
